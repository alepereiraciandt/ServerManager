...
public class ServerResource {
    ...
    public ResponseEntity<Response> getServers() {
        return ResponseEntity.ok(
                Response.builder()
                        .localDateTime(now())
                        .data(of("servers", serverServiceImp.list(30)))
    ...
    public ResponseEntity<Response> pingServer(@RequestBody @Valid Server server) {
        return ResponseEntity.ok(
                Response.builder()
                        .localDateTime(now())
                        .data(of("server", serverServiceImp.create(server)))
    ...
    public ResponseEntity<Response> getServer(@PathVariable("id") Long id) {
        return ResponseEntity.ok(
                Response.builder()
                        .localDateTime(now())
                        .data(of("server", serverServiceImp.get(id)))
                        .message("Servers retrieved")
                        .status(OK)
                        .statusCode(OK.value())
                        .build()
    ...
    public ResponseEntity<Response> deleteServer(@PathVariable("id") Long id) {
        return ResponseEntity.ok(
                Response.builder()
                        .localDateTime(now())
                        .data(of("deleted", serverServiceImp.delete(id)))
    ...
    public byte[] getServerImage(@PathVariable("fileName") String fileName) throws IOException {
        return readAllBytes(Paths.get(System.getProperty("user.home") + "Downloads/images/" + fileName));
    }
}
